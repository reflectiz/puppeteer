...
class Connection extends EventEmitter {
  ...
  static async create(url, delay = 0) {
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        ...
        resolve(new Connection(url, ws, delay));
      });
      ws.on('error', reject);
    });
  }
  ...
  _onConnectionError(err) {
    // cleanup the callbacks before sending the error event:
    this._onClose();
    this.emit('error', err);
  }
  ...
  async createSession(targetId) {
    ...
    return this.adoptSession(targetId, sessionId);
  }

  adoptSession(targetId, sessionId) {
