...
class Browser extends EventEmitter {
    ...
    constructor(connection, options = {}, process, closeCallback) {
        ...
        this._connection.on('error', this._onConnectionError.bind(this));
    ...
    _onConnectionError(err) {
        // we're not going to call the closeCallback if we closed due to an error
        if (this.listenerCount(Browser.Events.ConnectionError) === 0) throw err;

        this.emit(Browser.Events.ConnectionError, err);
    }
    ...
    async newPageInFreshContext() {
        ...
        return {
            ...
            page : this._newPageInContext(browserContextId)
        };
    }

    async _newPageInContext(context) {
        const {targetId} = await this._connection.send('Target.createTarget', {
            url: 'about:blank',
            browserContextId: context || undefined
        });
     ...
     */
    async newPage() {
        return this._newPageInContext(undefined);
    }
